...
def about(topic):
    ...
    def hastopic(paragraph):
        paragraph_new_list = remove_punctuation(paragraph).split()
        for i in paragraph_new_list:
            counter = 0
            while counter < len(topic):
                if lower(i) == lower(topic[counter]):
                    return True
                else:
                    counter+=1
        return False
...
def accuracy(typed, reference):
    ...
    for i in typed_words:
        if counter < len(reference_words):
            if i == reference_words[counter]:
                total = total + 1
                counter+=1
            else:
                counter+=1
...
def wpm(typed, elapsed):
    ...
    formula = len(typed) / 5
    time = 60/elapsed 
    combo_words_time = formula * time
    return float(combo_words_time)
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        for i in word_list:
            ...
            newList.append(x)
        min_diff = min(newList)
        min_diff_index = newList.index(min_diff)
        if min_diff <= limit:
...
def sphinx_swaps(start, goal, limit):
    ...
    return sphinx_swaps(start[1:], goal[1:], limit-add_on) + add_on
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    if start == goal:  # Fill in the condition
        ...
        return 1 + min(substitute, add, remove)
        # END
...
def time_per_word(words, times_per_player):
    ...
    for i in times_per_player:
        otherList.append([i[x] - i[x-1] for x in range(1, len(i))]) #List comprehension within for loop that appends to 'otherList'

    return match(words, otherList)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for x in word_indices:
        p = 0  
        tinytime = float(1000000)
        for y in player_indices:
            if time(match, y, x) < tinytime:
                tinytime = time(match, y, x)
                p = y
        fast_List[p].append(word_at(match, x))
